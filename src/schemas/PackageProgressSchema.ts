...
import { z } from 'zod';
import { DistillationItemSchema } from './DistillationItemSchema';
import { SourceItemSchema } from './SourceItemSchema';
...
const PackageStatsSchema = z.object({
  // Source statistics
  total_sources: z.number().int().nonnegative().describe('Total number of sources identified'),
  sources_collected: z
    .number()
    .int()
    .nonnegative()
  ...
  distillations_failed: z.number().int().nonnegative().describe('Number of failed distillations'),

  // Progress percentages
  collection_progress: z.number().min(0).max(100).describe('Collection progress percentage'),
  distillation_progress: z.number().min(0).max(100).describe('Distillation progress percentage'),
  overall_progress: z.number().min(0).max(100).describe('Overall package progress percentage'),
  ...
  // Timestamps
  started_at: z.string().datetime().optional().describe('When package processing started'),
  last_updated_at: z.string().datetime().optional().describe('Last update timestamp'),
  estimated_completion: z.string().datetime().optional().describe('Estimated completion time'),
});
...
const PackageConfigSchema = z.object({
  ...
  output_format: z.enum(['json', 'yaml']).default('json').describe('Format for distilled output'),
...
export const PackageProgressSchema = z.object({
  ...
  dependencies: z
    .object({
      required_tools: z.array(z.string()).optional().describe('Required tools for processing'),
      ai_agents: z.array(z.string()).optional().describe('AI agents used in this package'),
      external_apis: z
        .array(z.string())
        .optional()
  ...
  validation: z
    .object({
      schema_version: z.string().describe('Version of schemas used for validation'),
      validation_errors: z.array(z.string()).optional().describe('Validation errors encountered'),
      quality_checks_passed: z.boolean().optional().describe('Whether quality checks passed'),
      last_validated_at: z
        .string()
        .datetime()
        .optional()
  ...
  tags: z.array(z.string()).optional().describe('Tags for categorizing this package'),
});
