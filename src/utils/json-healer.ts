...
class JSONHealingStats {
  ...
  recordAttempt(result: RepairResult, originalError?: string) {
    ...
    if (result.valid) {
      ...
      if (result.strategy) {
        this.stats.strategyUsage[
          result.strategy as keyof typeof this.stats.strategyUsage
        ]++;
      }
    } else {
      ...
      // Track error types
      const errorKey = this.categorizeError(
        originalError || result.error || 'unknown'
      );
      this.stats.errorTypes.set(
        errorKey,
        (this.stats.errorTypes.get(errorKey) || 0) + 1
      );
    }
  }
  ...
  private categorizeError(error: string): string {
    ...
    if (errorLower.includes('unterminated string'))
      return 'unterminated-string';
    if (errorLower.includes('unterminated comment'))
  ...
  getSummary() {
    ...
    const successRate = Math.round(
      ((this.stats.alreadyValid + this.stats.successfulHealing) / total) * 100
    );
    const healingSuccessRate =
      this.stats.successfulHealing > 0
        ? Math.round(
            (this.stats.successfulHealing /
              (this.stats.successfulHealing + this.stats.failedHealing)) *
              100
          )
        : 0;
    ...
    const errorEntries = Array.from(this.stats.errorTypes.entries()).sort(
      ([, a], [, b]) => b - a
    );
    ...
    return {
      ...
      healingNeededRate: Math.round(
        ((this.stats.successfulHealing + this.stats.failedHealing) / total) *
          100
      ),
      healingSuccessRate,
  ...
  logPeriodicSummary() {
    ...
    if (this.stats.totalAttempts >= 10) {
      logger.debug(
        '📊 Detailed JSON Healing Strategy Breakdown',
...
export function healJSON(jsonString: string): RepairResult {
  ...
  logger.warn(
    `JSON healing failed - all strategies exhausted. Final error: ${finalValidation.error}`
  );
...
function generateRecommendations(summary: any): string[] {
  ...
  if (summary.overallSuccessRate < 80) {
    recommendations.push(
      'Overall success rate is low - consider improving AI prompt quality or output format'
    );
  }

  // Strategy effectiveness recommendations
  if (
    summary.mostEffectiveStrategy === 'jsonrepair' &&
    summary.mostEffectiveStrategyCount > summary.totalAttempts * 0.4
  ) {
    recommendations.push(
      'jsonrepair is highly effective - most JSON issues are minor formatting problems'
    );
  } else if (summary.mostEffectiveStrategy === 'dirty-json') {
    recommendations.push(
      'dirty-json is most effective - AI output has significant formatting issues that need tolerance'
    );
  } else if (
    summary.mostEffectiveStrategy === 'custom-newline' ||
    summary.mostEffectiveStrategy === 'custom-full'
  ) {
    recommendations.push(
      'Custom strategies are most needed - consider improving AI prompts to reduce complex JSON issues'
    );
  }
  ...
  if (summary.mostCommonError === 'unexpected-token') {
    recommendations.push(
      'Most failures due to unexpected tokens - review AI output for malformed JSON structure'
    );
  } else if (summary.mostCommonError === 'unterminated-string') {
    recommendations.push(
      'Most failures due to unterminated strings - AI may be generating incomplete responses'
    );
  } else if (summary.mostCommonError === 'newline-issue') {
    recommendations.push(
      'Newline issues are common - consider updating AI prompts to avoid unescaped newlines'
    );
  }
  ...
  if (summary.healingNeededRate > 50) {
    recommendations.push(
      'High healing rate indicates AI output quality could be improved with better prompts'
    );
  } else if (summary.healingNeededRate < 10) {
    recommendations.push(
      'Low healing rate indicates good AI output quality - JSON healing system is working well'
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      'JSON healing statistics look healthy - no immediate issues detected'
    );
  }
