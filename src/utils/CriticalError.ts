...
import type { ISourceItem } from '../modules/discovery/SourceItemFactory';
...
export class PipelineValidator {
  ...
  static validateDiscoveryOutput(data: PipelineData) {
    if (!data.sources || Object.keys(data.sources).length < 10) {
      throw new CriticalError(
        ...
        'discovery',
        {
          ...
          recommendedAction: 'Check source configurations and network connectivity',
        }
      );
    }
    ...
    const hasEventBus = Object.values(data.sources).some(
      (s) =>
        ...
        (s.id && s.id.includes('eventbus_migration_guide')) ||
        (s.title && s.title.toLowerCase().includes('eventbus'))
    );
    ...
    if (!hasEventBus) {
      const sourceIds = Object.values(data.sources)
        .map((s) => s.id || s.url)
        .slice(0, 5);
      throw new CriticalError('Missing critical EventBus 7 Migration Guide', 'discovery', {
        impact: 'Critical Forge porting information unavailable',
        evidence: `EventBus guide not found. Sample source IDs: ${sourceIds.join(', ')}`,
        recommendedAction: 'Verify EventBus guide URL in source configurations',
      });
    }
  }
  ...
  static validateCollectionOutput(data: PipelineData) {
    ...
    const failureRate =
      sources.length > 0 ? (sources.length - collectedSources.length) / sources.length : 1;

    if (collectedSources.length === 0) {
      throw new CriticalError('Collection phase produced ZERO successful downloads', 'collection', {
    ...
    if (failureRate > 0.5) {
      throw new CriticalError(
        ...
        'collection',
        {
          ...
          recommendedAction: 'Investigate collection errors and fix systematic issues',
        }
      );
    }
  }
  ...
  static validateDistillationOutput(data: PipelineData) {
    ...
    if (distilledSources.length === 0 && collectedSources.length > 0) {
      throw new CriticalError(
        ...
        'distillation',
        {
          ...
          recommendedAction: 'Check Gemini CLI availability and AI processing errors',
        }
      );
    }
    ...
    const failureRate =
      collectedSources.length > 0
        ? (collectedSources.length - distilledSources.length) / collectedSources.length
        : 0;
    ...
    if (failureRate > 0.5) {
      throw new CriticalError(
        ...
        'distillation',
        {
          ...
          recommendedAction: 'Check AI model availability and content preprocessing',
        }
      );
    }
  }
  ...
  static validatePackagingOutput(data: any) {
    ...
    if (!data.package_metadata.package_path) {
      throw new CriticalError('Packaging phase created no package directory', 'packaging', {
        impact: 'No package structure available for bundling',
        evidence: 'package_path is missing from metadata',
        recommendedAction: 'Verify packaging creates proper PACKAGED_DATA_MODEL structure',
      });
    }
  }
  ...
  static validateBundleOutput(data: any) {
    ...
    if (
      ...
      throw new CriticalError('Bundle phase failed to create distribution archive', 'bundling', {
