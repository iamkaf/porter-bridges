...
export class PerformanceMonitor {
  ...
  updateMetrics(
    ...
    if (!existing) {
      logger.warn(
        `No performance monitoring found for operation: ${operationName}`
      );
      return;
    }
    ...
    if (updates.itemsProcessed !== undefined) {
      ...
      existing.itemsPerSecond =
        elapsed > 0 ? existing.itemsProcessed / elapsed : 0;
    }
  }
  ...
  endMonitoring(
    ...
    metrics.itemsPerSecond =
      durationSeconds > 0 ? metrics.itemsProcessed / durationSeconds : 0;
  ...
  compareBenchmarks(
    ...
    const durationImprovement =
      bench1.metrics.duration && bench2.metrics.duration
        ? ((bench1.metrics.duration - bench2.metrics.duration) /
            bench1.metrics.duration) *
          100
        : 0;

    const memoryImprovement =
      ((bench1.metrics.memoryUsage.heapUsed -
        bench2.metrics.memoryUsage.heapUsed) /
        bench1.metrics.memoryUsage.heapUsed) *
      100;

    const throughputImprovement =
      bench1.metrics.itemsPerSecond && bench2.metrics.itemsPerSecond
        ? ((bench2.metrics.itemsPerSecond - bench1.metrics.itemsPerSecond) /
            bench1.metrics.itemsPerSecond) *
          100
        : 0;
  ...
  generateReport(): {
    ...
    if (this.benchmarks.length === 0) {
      return {
        ...
        benchmarks: [],
        recommendations: [
          'No benchmarks available. Start monitoring operations to gather performance data.',
        ],
      };
    }

    const validBenchmarks = this.benchmarks.filter((b) => b.metrics.duration);
    const totalDuration = validBenchmarks.reduce(
      (sum, b) => sum + (b.metrics.duration || 0),
      0
    );
    const totalMemory = this.benchmarks.reduce(
      (sum, b) => sum + b.metrics.memoryUsage.heapUsed,
      0
    );
    const totalThroughput = this.benchmarks.reduce(
      (sum, b) => sum + (b.metrics.itemsPerSecond || 0),
      0
    );
    ...
    const summary = {
      ...
      averageDuration:
        validBenchmarks.length > 0 ? totalDuration / validBenchmarks.length : 0,
    ...
    if (summary.averageDuration > 60_000) {
      recommendations.push(
        'Consider enabling parallel processing to reduce operation duration'
      );
    }

    if (summary.averageMemoryUsage > 500 * 1024 * 1024) {
      recommendations.push(
        'High memory usage detected. Consider enabling streaming for large operations'
      );
    }

    if (summary.averageThroughput < 1) {
      recommendations.push(
        'Low throughput detected. Consider optimizing batch sizes or enabling caching'
      );
    }
