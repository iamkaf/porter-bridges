...
import { PipelineStateSchema } from '../schemas/PipelineStateSchema';
...
export class PipelineStateManager {
  ...
  async loadState(): Promise<PipelineState> {
    try {
      ...
      if (!validation.success) {
        logger.warn('⚠️  Pipeline state validation failed, will attempt to fix', {
  ...
  async saveState(): Promise<void> {
    if (!this.state) {
      throw new Error('No state to save - call loadState() or initializeState() first');
    }
  ...
  updateSourcesFromLegacy(legacyData: any, phase: string): void {
    ...
    logger.info(`🔄 Updated ${Object.keys(sources).length} sources from legacy ${phase} data`);
  }
  ...
  addSources(sources: any): void {
    ...
    logger.info(`➕ Added ${Object.keys(sources).length} new sources to pipeline state`);
  }
  ...
  updateMetadata(): void {
    ...
    for (const source of Object.values(sources)) {
      if (phaseCounts.hasOwnProperty(source.status)) {
    ...
    const completedSources =
      phaseCounts.collected + phaseCounts.distilled + phaseCounts.packaged + phaseCounts.bundled;
