...
// @ts-expect-error - natural doesn't have perfect types
import { PorterStemmer, SentimentAnalyzer, TfIdf } from 'natural';
...
export class MLContentAnalyzer {
  ...
  async analyzeContent(content: ContentItem): Promise<ContentAnalysisResult> {
    ...
    const breakingChangesLikelihood =
      this.calculateBreakingChangesLikelihood(text);
  ...
  private extractKeywords(text: string): string[] {
    ...
    const domainKeywords = terms.filter(
      (term) =>
  ...
  private classifyContentType(
    text: string,
    sourceType: string
  ): ContentAnalysisResult['content_type'] {
    ...
    if (lowerText.includes('tutorial') || lowerText.includes('how to'))
      return 'tutorial';
    if (lowerText.includes('release') || lowerText.includes('version'))
      return 'changelog';
    if (lowerText.includes('guide') || lowerText.includes('migration'))
      return 'guide';
    if (lowerText.includes('announce') || lowerText.includes('news'))
      return 'announcement';
    if (lowerText.includes('documentation') || lowerText.includes('reference'))
  ...
  private calculateAdvancedRelevance(
    ...
    if (
      content.loader_type &&
      ['fabric', 'neoforge', 'forge'].includes(content.loader_type)
    ) {
      score += 0.1;
    }
    ...
    const domainTermCount = words.filter(
      (word) =>
  ...
  private calculateBreakingChangesLikelihood(text: string): number {
    ...
    if (lowerText.includes('migration') || lowerText.includes('porting'))
      score += 0.3;
   ...
   */
  getContentRecommendations(userPreferences: {
    minecraft_version?: string;
    loader_type?: string;
    ...
    return this.knowledgeBase
      .filter((content) => {
        if (
          userPreferences.minecraft_version &&
          content.minecraft_version !== userPreferences.minecraft_version
        ) {
          return false;
        }
        if (
          userPreferences.loader_type &&
          content.loader_type !== userPreferences.loader_type
        ) {
          return false;
        }
        if (
          userPreferences.content_types &&
          !userPreferences.content_types.includes(content.source_type)
        ) {
          return false;
        }
        return true;
