...
export class CollectionStats {
  ...
  incrementCollected(bytes: number = 0): void {
  ...
  getSummary(): ICollectionSummary {
    const durationMs =
      this.stats.collection_end_time && this.stats.collection_start_time
        ? new Date(this.stats.collection_end_time).getTime() - new Date(this.stats.collection_start_time).getTime()
        : 0;
    ...
    return {
      ...
      success_rate:
        this.stats.total_sources > 0
          ? Math.round((this.stats.collected_sources / this.stats.total_sources) * 100)
          : 0,
