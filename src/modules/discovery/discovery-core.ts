...
export class DiscoveryCore {
  ...
  async discover() {
    ...
    try {
      ...
      logger.info(
        `🎯 Discovery complete! Found ${this.discoveredSources.size} sources`
      );
  ...
  _getDiscoveryResults() {
    return {
      ...
      summary: {
        ...
        source_types: [
          ...new Set(
            Array.from(this.discoveredSources.values()).map(
              (s) => s.source_type
            )
          ),
        ],
  ...
  getSourcesByCriteria(
    ...
    return sources.filter((source) => {
      ...
      if (
        criteria.minecraft_version &&
        source.minecraft_version !== criteria.minecraft_version
      ) {
        return false;
      }
      ...
      if (
        ...
        source.relevance_score < (criteria.min_relevance as number)
      ) {
        return false;
      }
  ...
  _validateDiscoveryResults(
    ...
    if (expectedMin && discoveredCount < expectedMin) {
      ...
      logger.warn(
        '   This may indicate a parser issue or source format change'
      );
    }
