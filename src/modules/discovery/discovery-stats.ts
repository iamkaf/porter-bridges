...
export class DiscoveryStats {
  stats: {
    ...
    discovery_start_time: string | null;
    discovery_end_time: string | null;
  };
  ...
  printDiscoveryStats(discoveredSources: Map<string, ISourceItem>) {
    let duration = 0;
    if (this.stats.discovery_end_time && this.stats.discovery_start_time) {
      duration =
        new Date(this.stats.discovery_end_time).getTime() -
        new Date(this.stats.discovery_start_time).getTime();
    }
