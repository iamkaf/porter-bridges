...
import { httpClient, createHttpError } from '../../utils/http';
...
export class DirectUrlDiscovery {
  ...
  async discover(sourceConfig: ISourceConfig) {
    ...
    try {
      ...
      logger.error({ error: httpError.message }, 'Failed to discover from direct URL');
      throw httpError;
    }
   ...
   */
  _generateTitle(sourceConfig: ISourceConfig, urlInfo: any): string {
    ...
    } else if (urlInfo.type === 'github_page') {
      return `${urlInfo.owner}/${urlInfo.repo} Guide`;
    } else if (urlInfo.type === 'documentation') {
   ...
   */
  _extractMinecraftVersion(sourceConfig: ISourceConfig, _urlInfo: any): string | null {
    // Check if explicitly mentioned in description
    if (sourceConfig.description) {
      const versionMatch = sourceConfig.description.match(/\b1\.\d+(?:\.\d+)?\b/);
      if (versionMatch) {
    ...
    if (sourceConfig.url.includes('PaintNinja') && sourceConfig.url.includes('ad82c224')) {
  ...
  _generateTags(sourceConfig: ISourceConfig, urlInfo: any): string[] {
    ...
    const minecraftVersion = this._extractMinecraftVersion(sourceConfig, urlInfo);
    if (minecraftVersion) {
  ...
  _calculateRelevance(sourceConfig: ISourceConfig): number {
    ...
    if (sourceConfig.url.includes('PaintNinja') && sourceConfig.url.includes('eventbus')) {
      relevance = 1.0;
    }
