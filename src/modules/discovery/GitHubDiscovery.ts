...
export class GitHubDiscovery {
   ...
   */
  async discoverFromGitHubDirectory(sourceId: string, config: ISourceConfig, discoveredSources: Map<string, any>): Promise<number> {
    try {
      const files = await githubClient.get(config.url).json<Array<{
        name: string;
        type: 'file' | 'dir';
        download_url?: string;
        url: string;
    ...
    for (const item of files) {
      ...
      if (item.type === 'dir') {
        ...
        const sourceItem = await this.sourceItemFactory.createSourceItem({
          ...
          relevance_score: this.contentAnalyzer.calculateRelevance(version, config.source_type),
        });
      ...
      logger.error({ error: httpError.message }, 'Failed to discover from GitHub directory');
      throw httpError;
    }
  }
