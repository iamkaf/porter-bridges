...
import { rssClient } from '../../utils/http';
...
export class RSSDiscovery {
  ...
  async discoverFromRSSFeed(
    ...
    return executeWithDegradation(
      async () => {
        const rssContent = await rssClient.getText(config.url);
      ...
      'rss_discovery',
      `discover_from_rss_feed_${sourceId}`,
      {
      ...
      if (result.degraded) {
        logger.warn(
          `🔄 RSS discovery completed with degradation`,
          {
            sourceId,
