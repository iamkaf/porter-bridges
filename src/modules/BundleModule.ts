...
import { BundleStats } from './bundling/BundleStats';
...
export class BundleModule {
  ...
  async bundle(options: Record<string, any> = {}) {
    ...
    try {
      ...
      logger.info('📋 Found packages to bundle', { count: availablePackages.length });
      ...
      for (const packageInfo of availablePackages) {
        try {
          ...
          const packageData = await this._bundleSinglePackage(packageInfo, bundlePath);
          ...
          logger.error(`❌ Bundling failed: ${packageInfo.version} - ${error.message}`);
        }
      }
      ...
      if (this.options.createArchive) {
        archivePath = await this._createDistributionArchive(bundlePath, bundleName);
      }
  ...
  async _findAvailablePackages(): Promise<Array<{ version: string; path: string; manifestPath: string }>> {
  ...
  async _bundleSinglePackage(packageInfo: { version: string; path: string; manifestPath: string }, bundlePath: string) {
    try {
      ...
      logger.error(`Failed to bundle package: ${packageInfo.version}`, { error: error.message });
      throw error;
    }
  }

  _generateBundleManifest(bundledData: Record<string, any>, bundleName: string) {
    ...
    for (const [_version, data] of Object.entries(bundledData)) {
      ...
      if (typedData.manifest && typedData.manifest.content_breakdown) {
        for (const [type, count] of Object.entries(typedData.manifest.content_breakdown)) {
          contentSummary[type as string] = (contentSummary[type as string] || 0) + (count as number);
        }
      }
    }
  ...
  async _generateBundleChecksums(bundlePath: string) {
    ...
    try {
      ...
      logger.warn('Failed to generate bundle checksums', { error: error.message });
    }
  ...
  _createDistributionArchive(bundlePath: string, bundleName: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        output.on('close', async () => {
          try {
            ...
            const compressionRatio =
              originalSize > 0 ? Math.round((1 - archiveStats.size / originalSize) * 100) : 0;
        ...
        logger.info('📁 Adding bundle directory to ZIP archive', { bundlePath });
  ...
  _buildResults(options: any, bundlePath: string | null = null, archivePath: string | null = null) {
    return {
      bundle_metadata: {
        bundled_at: new Date().toISOString(),
