...
export class CrossReferenceAnalyzer {
   ...
   */
  calculateRelationshipScore(item1: ICrossReferenceItem, item2: ICrossReferenceItem): number {
    let score = 0;

    // API overlap (weighted heavily)
    const apis1 = new Set((item1.affected_apis || []).map((api) => api.toLowerCase()));
    const apis2 = new Set((item2.affected_apis || []).map((api) => api.toLowerCase()));
   ...
   */
  determineRelationshipType(item1: ICrossReferenceItem, item2: ICrossReferenceItem, score: number): string {
    const apis1 = new Set((item1.affected_apis || []).map((api) => api.toLowerCase()));
    const apis2 = new Set((item2.affected_apis || []).map((api) => api.toLowerCase()));
    const commonApis = new Set([...apis1].filter((x) => apis2.has(x)));
  ...
  addCrossReferences(items: ICrossReferenceItem[]): ICrossReferenceItem[] {
    ...
    for (let i = 0; i < withCrossRefs.length; i++) {
      ...
      for (let j = 0; j < withCrossRefs.length; j++) {
        ...
        const relationshipScore = this.calculateRelationshipScore(currentItem, otherItem);
   ...
   */
  processWithCrossReferences(items: ICrossReferenceItem[], itemType: string): ICrossReferenceItem[] {
    if (!items || items.length === 0) {
      return [];
    }

    logger.info(`🔗 Processing ${itemType} with cross-referencing and de-duplication`, {
      count: items.length,
    });
   ...
   */
  private deduplicateItems(items: ICrossReferenceItem[]): ICrossReferenceItem[] {
    ...
    for (const item of items) {
      ...
      if (seen.has(key)) {
        ...
        if (item.affected_apis) {
          existing.affected_apis = [...new Set([...(existing.affected_apis || []), ...item.affected_apis])];
        }
  ...
  private generateDeduplicationKey(item: ICrossReferenceItem): string {
    ...
    const apis = (item.affected_apis || []).map((api) => api.toLowerCase().trim()).sort();
