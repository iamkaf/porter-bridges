  ...
  getSummary(): IPackageSummary {
    const durationMs =
      this.stats.package_end_time && this.stats.package_start_time
        ? new Date(this.stats.package_end_time).getTime() - new Date(this.stats.package_start_time).getTime()
        : 0;
    ...
    return {
      ...
      success_rate:
        this.stats.total_sources > 0
          ? Math.round((this.stats.packaged_sources / this.stats.total_sources) * 100)
          : 0,
