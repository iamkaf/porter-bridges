...
class CollectionFilters {
  filterSources(sourcesData: PipelineState, filters: any): PipelineSourceType[] {
    ...
    return sources.filter((source) => {
      ...
      if (filters.includeRetry && source.status !== 'discovered' && source.status !== 'failed') {
        return false;
      }
...
class ContentDownloader {
  ...
  async downloadContent(url: string) {
    ...
    if (response.statusCode < 200 || response.statusCode >= 400) {
      throw new Error(`HTTP ${response.statusCode}: ${response.statusCode || 'Request failed'}`);
    }
...
export class CollectionModule {
   ...
   */
  async collect(sourcesData: PipelineState, filters = {}) {
    ...
    try {
      ...
      for (let i = 0; i < sources.length; i++) {
        ...
        try {
          ...
          logger.error(`❌ Collection failed: ${source.url} - ${error.message}`);
        }
      }
