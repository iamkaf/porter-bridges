...
export class GeminiProcessor {
  ...
  async distillSingleSource(source: any, task: any) {
    ...
    try {
      ...
      const contentChecksum = crypto.createHash('sha256').update(rawContent).digest('hex');
      ...
      logger.info(`🔄 Processing: ${absoluteContentPath} → ${absoluteOutputPath}`);
      ...
      try {
        ...
        distilledContent = await this.responseParser.parseFileContent(fileContent);
      } catch (error: any) {
        parseError = new Error(`Failed to read or parse generated file: ${error.message}`);

        // If JSON parsing failed, delete the bad file so it can be re-processed
        if (error.message.includes('parse JSON') || error.message.includes('JSON')) {
          try {
            await fs.unlink(absoluteOutputPath);
            logger.warn(`🗑️  Deleted invalid JSON file: ${absoluteOutputPath}`);
          } catch (deleteError) {
            logger.warn(`Failed to delete invalid file: ${deleteError.message}`);
          }
        }
      }
      ...
      const updatedSource = {
        ...
        distillation_metadata: {
          ...
          confidence_score: distilledContent.quality_metrics?.accuracy_confidence || 0.8,
  ...
  _callGeminiCLI(prompt: string): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      geminiProcess.on('error', (error) => {
        ...
        const enhancedError = new Error(`Failed to start Gemini CLI: ${error.message}`) as any;
  ...
  _parseTokenUsage(stderr: string) {
    try {
      ...
      if (inputMatch || outputMatch) {
        return {
          input_tokens: inputMatch ? parseInt(inputMatch[1]) : 0,
          output_tokens: outputMatch ? parseInt(outputMatch[1]) : 0,
          total_tokens:
            (inputMatch ? parseInt(inputMatch[1]) : 0) +
            (outputMatch ? parseInt(outputMatch[1]) : 0),
        };
      }
    } catch (error: any) {
