...
import { DistilledContentSchema } from '../../schemas/DistilledContentSchema';
...
export class ResponseParser {
  ...
  parseFileContent(fileContent: string) {
    try {
      ...
      throw new Error(`Failed to parse JSON from file content: ${error.message}`);
    }
  }
  ...
  parseGeminiResponse(output: string) {
    try {
      ...
      logger.error({ error: error.message, output }, '❌ Failed to parse Gemini response');
