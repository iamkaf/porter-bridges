...
import { PackageStats } from './packaging/PackageStats';
import { CrossReferenceAnalyzer } from './packaging/CrossReferenceAnalyzer';
import { VersionGrouper } from './packaging/VersionGrouper';
import { ContentProcessor } from './packaging/ContentProcessor';
...
export class PackageModule {
  ...
  async package(sourcesData: PipelineState, options = {}) {
    logger.info('📦 Starting packaging process (PACKAGED_DATA_MODEL compliant)');
    ...
    try {
      ...
      logger.info('📋 Found distilled sources to package', { count: distilledSources.length });
      ...
      logger.info('📁 Created package structure', { packagePath, rawPath, distilledPath });

      // Process sources and organize by version
      const versionGroups = this.versionGrouper.groupSourcesByVersion(distilledSources);
      const packageResults: { sources: Record<string, any>; distillation: Record<string, any> } = { sources: {}, distillation: {} };

      for (const [version, sources] of Object.entries(versionGroups)) {
        const versionPath = path.join(distilledPath, version);
        await fs.mkdir(versionPath, { recursive: true });

        logger.info(`📂 Processing version: ${version}`, { sourceCount: sources.length });
        ...
        for (const source of sources) {
          try {
            const result = await this.contentProcessor.processSourceForVersion(source, rawPath, versionPath);

            if (result) {
              packageResults.sources[this.contentProcessor.generateSourceKey(source)] = result.sourceProgress;
              packageResults.distillation[this.contentProcessor.generateSourceKey(source)] =
              ...
              if (result.distilledData.breaking_changes) {
                const attributedBreakingChanges = result.distilledData.breaking_changes.map(
                  (item: any) => ({
                    ...item,
              ...
              if (result.distilledData.api_updates) {
                const attributedApiUpdates = result.distilledData.api_updates.map((item: any) => ({
                  ...item,
                  source_url: source.url,
                }));
              ...
              if (result.distilledData.migration_guides) {
                const attributedMigrationGuides = result.distilledData.migration_guides.map(
                  (item: any) => ({
                    ...item,
                    source_url: source.url,
                  })
              ...
              if (result.distilledData.dependency_updates) {
                const attributedDependencyUpdates = result.distilledData.dependency_updates.map(
                  (item: any) => ({
                    ...item,
                    source_url: source.url,
                  })
            ...
            logger.error(`❌ Processing failed: ${source.url} - ${error.message}`);

            // Add failed entry to progress tracking
            packageResults.sources[this.contentProcessor.generateSourceKey(source)] = {
              status: 'failed',
              url: source.url,
              error: error.message,
        ...
        const processedBreakingChanges = this.crossReferenceAnalyzer.processWithCrossReferences(
          breakingChanges,
          'breaking_change'
        );
        const processedApiUpdates = this.crossReferenceAnalyzer.processWithCrossReferences(apiUpdates, 'api_update');

        // Write categorized content for version
        if (processedBreakingChanges.length > 0) {
          const breakingChangesPath = path.join(versionPath, 'breaking-changes.json');
          await fs.writeFile(
            breakingChangesPath,
            JSON.stringify(
              {
                ...
                processing_metadata: {
                  ...
                  deduplicated_count: breakingChanges.length - processedBreakingChanges.length,
          ...
          logger.info(`📝 Wrote breaking changes for ${version}`, {
            ...
            deduplicated: breakingChanges.length - processedBreakingChanges.length,
          });
        }
            ...
            JSON.stringify(
              {
                ...
                processing_metadata: {
                  ...
                  deduplicated_count: apiUpdates.length - processedApiUpdates.length,
        ...
        if (migrationGuides.length > 0) {
          const migrationGuidesPath = path.join(versionPath, 'migration-guides.json');
          await fs.writeFile(
            migrationGuidesPath,
            JSON.stringify(
              {
        ...
        if (dependencyUpdates.length > 0) {
          const processedDependencyUpdates = this.crossReferenceAnalyzer.processWithCrossReferences(
            dependencyUpdates,
            'dependency_update'
          );
          const dependencyUpdatesPath = path.join(versionPath, 'dependency-updates.json');
          await fs.writeFile(
            ...
            JSON.stringify(
              {
                ...
                processing_metadata: {
                  ...
                  deduplicated_count: dependencyUpdates.length - processedDependencyUpdates.length,
          ...
          logger.info(`📝 Wrote dependency updates for ${version}`, {
            ...
            deduplicated: dependencyUpdates.length - processedDependencyUpdates.length,
          });
        }
      ...
      const packageMetadata = await this._generatePackageMetadata(packageResults, sourcesData);
  ...
  _logSummary(packagePath: string): void {
    ...
    logger.info('📊 Packaging Summary (with Cross-Referencing & De-duplication)', {
      ...
      packagePath,
      enhancedFeatures: ['cross_referencing', 'deduplication', 'relationship_scoring'],
    });
  }
  ...
  async _generatePackageTree(packagePath: string): Promise<void> {
    try {
      ...
      logger.warn('⚠️  Failed to generate package tree structure', { error: error.message });
    }
  }
   ...
   */
  async _buildDirectoryTree(dirPath: string, rootPath: string, prefix: string = ''): Promise<string> {
    const items = [];
    try {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (entry.isDirectory()) {
          ...
          const subtree: string = await this._buildDirectoryTree(subdirPath, rootPath, prefix + nextPrefix);
          items.push(subtree);
        }
      }
    } catch (error: any) {
  ...
  _buildResults(sourcesData: any, options: any, packagePath: string | null = null): any {
    return {
