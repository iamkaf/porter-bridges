...
import { GeminiProcessor } from './distillation/GeminiProcessor';
...
class DistillationFilters {
  filterSources(sourcesData: any, filters: any) {
    ...
    for (const [sourceKey, source] of Object.entries(sourcesData.sources || {})) {
      ...
      if (filters.includeRetry && (source as any).status !== 'collected' && (source as any).status !== 'failed') {
        continue;
      }

      // Apply filters
      if (filters.sourceType && (source as any).source_type !== filters.sourceType) {
        continue;
      }

      if (filters.loaderType && (source as any).loader_type !== filters.loaderType) {
        continue;
      }
      ...
      if (filters.minRelevance && (source as any).relevance_score < filters.minRelevance) {
        continue;
      }
  ...
  getSummary() {
    const durationMs =
      this.stats.distillation_end_time && this.stats.distillation_start_time
        ? new Date(this.stats.distillation_end_time).getTime() - new Date(this.stats.distillation_start_time).getTime()
        : 0;
...
export class DistillationModule {
  ...
  async distill(sourcesData: any, filters: Record<string, any> = {}) {
    ...
    try {
      ...
      logger.info('📋 Distillation plan', { breakdown: this._getBreakdownCounts(sourcesByType) });

      // Estimate processing time
      const estimatedMinutes = Math.ceil(sources.length * 1); // ~1 minute per source
      logger.info('⏱️  Estimated processing time', { estimatedTimeMinutes: estimatedMinutes });
      ...
      for (let i = 0; i < sources.length; i++) {
        ...
        try {
          ...
          if (source._sourceKey && sourcesData.sources[source._sourceKey]) {
            ...
            delete sourcesData.sources[source._sourceKey]._sourceKey;
          }
        } catch (error: any) {
          ...
          logger.error(`❌ Distillation failed: ${source.url} - ${error.message}`);
        }
      }
